###  [v2.4.1] - September 23, 2024
Platform
- Added dispute management APIs. 
- Bug fixes.

Dashboard
- Watchdog 2.0 (in-flight transaction monitoring system).
- Dispute management system 2.0.
- Card config enhancements. 
- Improved list view filters. 
- Support to initiate debit card transactions.

###  [v2.4] - August 14, 2024
Platform
- Added simulate incoming wire APIs.
- Enhanced Watchdog 2.0(in-flight transaction monitoring system).
- Added bank info API.
- Added webhook APIs. 
- Real Time Authorization(RTA) support for ACH and Card transactions. 

Dashboard
- Support to simulate incoming domestic and international wire transactions.
- Unified login experience for sandbox and production environments
- Bug fixes.

###  [v2.3] - July 29, 2024
Platform
- Enabled v2 APIs for debit pull and push payment form factor.
- Started enforcing card controls for card transactions.
- Added ATM locations API.
- Enabled support for idempotency in v2.

Dashboard
- Enhanced counterparty module to add RTP, debit card and check deposit details.
- Support to originate wire, send check, check deposit, and debit pull/push payments from the dashboard.
- Added events, API logs and webhook support.
- Support for bank user.
- Improved sign-in screen.

### [v2.2] - July 15, 2024
Platform
- Support for API logs, events, and webhooks.
- Enabled v2 APIs for check deposit and RTP.
- Started enforcing configured limits for incoming and outgoing transactions.
- Enhanced API error codes.
- Business member API enhancements

Dashboard
- Support to originate intra account, FedNow and RTP payments from the dashboard.
- Ability to configure webhooks and subscribe to various events.
- Support to update transactions.
- Enhanced filters in accounts, account holders, counterparties, cards, and transactions screens.

### [v2.1] - July 4, 2024

Platform
- Enabled v2 APIs for domestic & international wire, send check and FedNow payment form factors.
- Added attachments APIs - https://docs.solidfi.com/v2/api-reference/attachments.

Dashboard
- Enhanced counterparty module to add wire, send check and FedNow payment details.
- Support to originate ACH payments from the dashboard.
- Ability to add attachments to accounts, accountholder, counterparties, cards, and transactions.
- Added card config at the master account level.
- The user role is now displayed on the user profile screen.
- Enhanced reset password flow. 


### [v2.0] - June 17, 2024
Launch of the Solid v2 Platform. Read more [here](https://www.solidfi.com/post/solid-v2-platform-arrives-to-power-a-new-era-of-embedded-fintech-innovation).
...
- Program name prefix when you Send an ACH: Solid `Program name`: `Person/Business name` `Transaction description`
